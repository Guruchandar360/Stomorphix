...
class BatchManager:
    ...
    def _process(self, job_id):
        ...
        for item in list(job["uploaded"]):
            ...
            try:
                result = analyze_image(
                    ...
                    display_name=item["relative_name"],
                )
        ...
        if job["results"]:
            ...
            try:
                ...
                job["explanation"] = detail if len(detail.split()) >= 8 else fallback
            except Exception:
    ...
    def _write_summary_csv(self, job):
        ...
        fields = ["filename", "stomata_count"]
        with path.open("w", newline="", encoding="utf-8") as handle:
            writer = csv.DictWriter(handle, fieldnames=fields, extrasaction="ignore")
            writer.writeheader()
            for result in job["results"]:
                writer.writerow({"filename": result["original_name"], "stomata_count": result["stomata_count"]})

    def _write_archives(self, job):
        job_dir = self._job_dir(job["id"])
        annotated_zip = job_dir / "annotated_images.zip"
        count_zip = job_dir / "count_reports.zip"
        ...
        with zipfile.ZipFile(count_zip, "w", zipfile.ZIP_DEFLATED) as archive:
            for result in job["results"]:
                path = self._url_to_path(result["count_report_url"])
                archive.write(path, arcname=path.name)
        ...
        with zipfile.ZipFile(complete_zip, "w", zipfile.ZIP_DEFLATED) as archive:
            for result in job["results"]:
                ...
                report = self._url_to_path(result["count_report_url"])
                archive.write(annotated, arcname=f"annotated_images/{annotated.name}")
                archive.write(report, arcname=f"count_reports/{report.name}")
            archive.write(job_dir / "summary.csv", arcname="summary.csv")
            ...
            results = [
                {
                    ...
                    "count_report_url": result["count_report_url"],
                }
    ...
    def _fallback_explanation(job):
        ...
        return (
            ...
            f"{summary['avg_stomata_per_image']} stomata per image."
        )
